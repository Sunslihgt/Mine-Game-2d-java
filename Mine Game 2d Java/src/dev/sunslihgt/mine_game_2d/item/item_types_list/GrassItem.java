...
public class GrassItem extends ItemType {

	public GrassItem(int id) {
		super(id, "grass", Assets.grass_item, 64, ToolType.NONE, 0, 1, 0f);
	}
	...
	public boolean rightClickAction(Handler handler) {
		...
		if (handler.getWorld().isEmptyBlock(playerCursorX, playerCursorY) && !handler.getPlayer().checkCollisionBlockPos(playerCursorX, playerCursorY)) {
			Block dirtBlock = new Block(playerCursorX, playerCursorY, BlockType.grassBlock);
