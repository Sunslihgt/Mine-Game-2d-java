...
public class OakWoodItem extends ItemType {

	public OakWoodItem(int id) {
		super(id, "oak wood", Assets.oak_wood_item, 64, ToolType.NONE, 0, 1, 0f);
	}
	...
	public boolean rightClickAction(Handler handler) {
		...
		if (handler.getWorld().isEmptyBlock(playerCursorX, playerCursorY) && !handler.getPlayer().checkCollisionBlockPos(playerCursorX, playerCursorY)) {
			Block block = new Block(playerCursorX, playerCursorY, BlockType.oakWoodBlock);
